...
# ═══════════════════════════════════════════════════════════════
# ═══════════════════════════════════════════════════════════════
# CELL 3 — Load & Prepare Data (SIDE-BY-SIDE SPLIT METHOD)
# ═══════════════════════════════════════════════════════════════
import pandas as pd
import gc

FILE_1 = "Main data file 1.xlsx - Sheet1.csv"
FILE_2 = "Main Data File 2.xlsx - Sheet1.csv"

try:
    # 1. Load the left half and right half of your data
    df_left = pd.read_csv(FILE_1, low_memory=False).dropna(how='all')
    df_right = pd.read_csv(FILE_2, low_memory=False).dropna(how='all')

    # If you accidentally copied 'country' and 'year' into the second file, remove them so they don't duplicate
    ...
    # 2. Stitch them together side-by-side!
    df_raw = pd.concat([df_left.reset_index(drop=True), df_right.reset_index(drop=True)], axis=1)

    # 3. Force the exact 17 columns your dashboard needs (grabs the first 17 cols)
    ...
    # Clean up empty ghost rows and format the year
    df_raw = df_raw.dropna(subset=['country'])
    df_raw['year'] = pd.to_numeric(df_raw['year'], errors='coerce')
    df = df_raw[df_raw['year'] >= 1990].copy()

    # 4. Extract ONLY our 3 targets to save RAM
    ...
    # 5. NUKE the massive global dataset from the server's RAM
    del df_left
    del df_right
    del df_raw
    del df
    ...
    japan, india, world = pd.DataFrame(), pd.DataFrame(), pd.DataFrame()
...
# Safe Getter prevents "Error loading layout" if data is missing
...
print("✅ Left & Right files zipped together perfectly!")
# ═══════════════════════════════════════════════════════════════
