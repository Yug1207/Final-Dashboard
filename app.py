...
# ═══════════════════════════════════════════════════════════════
# LOAD DATA  — total RAM footprint: ~2 MB (was 157 MB)
# ═══════════════════════════════════════════════════════════════

# Core 3-entity data (105 rows)
core = pd.read_csv("core_data.csv")
japan = core[core["country"] == "Japan"].reset_index(drop=True)
india = core[core["country"] == "India"].reset_index(drop=True)
world = core[core["country"] == "World"].reset_index(drop=True)

# Slim extra-country data for callbacks (country + year + co2_per_capita + co2_share + pop)
df_slim = pd.read_csv("countries_slim.csv")

# GDP 2023 lookup dict  {country: gdp_per_capita}
with open("gdp_2023.json") as f:
    _gdp_bubble = json.load(f)
...
india["co2_mt"] = india["co2_total"] / 1e9
india["renewables"] = (
    india["solar"] + india["wind"] + india["hydropower"] + india["other_renewables"]
)

# Scalar KPIs
co2_2023      = japan.loc[japan["year"] == 2023, "co2_mt"].values[0]
pc_2023       = japan.loc[japan["year"] == 2023, "co2_per_capita"].values[0]
pc_1990       = japan.loc[japan["year"] == 1990, "co2_per_capita"].values[0]
pct_change    = round(((pc_2023 - pc_1990) / pc_1990) * 100, 1)
share_2023    = japan.loc[japan["year"] == 2023, "co2_share"].values[0]
world_pc_2023 = world.loc[world["year"] == 2023, "co2_per_capita"].values[0]
india_pc_2023 = india.loc[india["year"] == 2023, "co2_per_capita"].values[0]
...
PLOTLY_LAYOUT = dict(
    ...
    legend=dict(
        bgcolor="rgba(0,0,0,0)", font=dict(color=SUBTEXT, size=11),
...
ENERGY_LABELS = {"coal": "Coal", "oil": "Oil", "gas": "Gas",
...
# ═══════════════════════════════════════════════════════════════
# STATIC CHARTS  (built once from the tiny japan/india/world frames)
# ═══════════════════════════════════════════════════════════════

def _vlines(fig, year_range, y_ref):
    y0, y1 = year_range if year_range else (1990, 2024)
    for yr, lbl in [(2011, "Fukushima"), (2016, "Paris Agmt")]:
        if y0 <= yr <= y1:
            fig.add_vline(x=yr, line_dash="dash", line_color=SUBTEXT,
                          line_width=1, opacity=0.5)
            fig.add_annotation(x=yr, y=y_ref, text=lbl, showarrow=False,
                font=dict(color=SUBTEXT, size=10, family=FONT_BODY), xshift=5, yshift=10)


fig_scatter = go.Figure()
fig_scatter.add_trace(go.Scatter(
    x=japan["gdp_per_capita"] if "gdp_per_capita" in japan.columns else [None],
    y=japan["co2_per_capita"],
    mode="markers+text", text=japan["year"].astype(str),
    textposition="top center", textfont=dict(color=SUBTEXT, size=9),
    marker=dict(
        size=10, color=japan["year"],
...
_bar = go.Figure(data=[go.Bar(
    x=["Japan", "World Average", "India"],
    y=[pc_2023, world_pc_2023, india_pc_2023],
...
_bar.update_layout(**PLOTLY_LAYOUT,
    title=dict(text="Per Capita CO₂ Comparison (2023)",
               font=dict(color=TEXT, size=14, family=FONT_HEAD)),
...
server = app.server  # ← required for gunicorn (Procfile: web: gunicorn app:server)
...
dashboard = html.Div(id="dashboard", style={
    ...
    html.Div(style={"marginBottom": "60px"}, children=[
        ...
        html.Div(style={"display": "grid", "gridTemplateColumns": "1fr 1fr", "gap": "24px"}, children=[
            chart_card(go.Figure(), gid="chart-total"),
            chart_card(go.Figure(), gid="chart-pc"),
        ]),
    ]),
    ...
    html.Div(style={"marginBottom": "60px"}, children=[
        ...
        html.Div(style={"background": CARD_BG, "border": f"1px solid {BORDER}",
            ...
            html.Div(style={"display": "flex", "alignItems": "center", "gap": "16px",
                ...
                dcc.Dropdown(
                    id="energy-dropdown",
                    options=[{"label": v, "value": k} for k, v in ENERGY_LABELS.items()],
        ...
        html.Div(style={"background": CARD_BG, "border": f"1px solid {BORDER}",
            ...
            html.Div(style={"display": "flex", "alignItems": "center", "gap": "16px", "flexWrap": "wrap"}, children=[
                ...
                dcc.Dropdown(
                    ...
                    value=[], multi=True, placeholder="Select countries to add...",
                    style={"flex": "1", "minWidth": "300px", "fontFamily": FONT_BODY, "fontSize": "12px"},
                ),
            ]),
        ]),
        html.Div(style={"display": "grid", "gridTemplateColumns": "2fr 1fr", "gap": "24px"}, children=[
            chart_card(go.Figure(), gid="chart-compare"),
            chart_card(go.Figure(), gid="chart-donut"),
        ]),
    ]),
...
def update_trends(year_range):
    ...
    _vlines(f1, year_range, j["co2_mt"].max() if not j.empty else 0)
    ...
    f2.add_trace(go.Scatter(x=j["year"], y=j["co2_per_capita"], mode="lines",
        ...
        hovertemplate="<b>%{x}</b><br>%{y:.2f} tCO₂/person<extra></extra>"))
    _vlines(f2, year_range, j["co2_per_capita"].max() if not j.empty else 0)
...
def update_compare(selected_countries, year_range):
    ...
    if selected_countries:
        for i, country in enumerate(selected_countries):
            d = df_slim[(df_slim["country"] == country) &
                        (df_slim["year"] >= y0) & (df_slim["year"] <= y1)]
            if not d.empty:
                fig.add_trace(go.Scatter(
...
def update_donut(selected_countries):
    all_c = selected_countries or []
    donut_countries = ["Japan"] + [c for c in all_c if c not in ("World", "India")]
    donut_colors = [RED] + EXTRA_COLORS[:len(donut_countries) - 1]

    shares, labels, colors_final = [], [], []
    for i, country in enumerate(donut_countries):
        if country == "Japan":
            row = japan[japan["year"] == 2023]
        else:
            row = df_slim[(df_slim["country"] == country) & (df_slim["year"] == 2023)]
        if not row.empty and not pd.isna(row["co2_share"].values[0]):
            shares.append(round(float(row["co2_share"].values[0]), 2))
    ...
    fig_d = go.Figure(go.Pie(
        ...
        marker=dict(colors=colors_final), textinfo="none",
        hovertemplate="<b>%{label}</b><br>%{value:.2f}%<extra></extra>",
    ))
    fig_d.add_annotation(
        text=f"<b>{combined_share}%</b>", x=0.5, y=0.56 if is_coalition else 0.5,
        showarrow=False, font=dict(color=TEXT, size=20, family=FONT_HEAD), align="center",
    )
    if is_coalition:
        fig_d.add_annotation(
            text=f"{len(labels) - 1} countries", x=0.5, y=0.40,
            showarrow=False, font=dict(color=SUBTEXT, size=11, family=FONT_BODY), align="center",
        )
    fig_d.update_layout(**PLOTLY_LAYOUT)
    fig_d.update_layout(
        title=dict(
            text="Combined Share of Global CO₂ (2023)" if is_coalition else "Japan's Share of Global CO₂ (2023)",
            font=dict(color=TEXT, size=14, family=FONT_HEAD),
        ),
        showlegend=True,
...
def update_bubble(selected_countries):
    ...
    for name, data, color, label in [
        ...
        if gdp is None or pd.isna(co2):
            continue
        fig.add_trace(go.Scatter(
            x=[gdp], y=[co2], mode="markers+text", name=label, text=[label],
            textposition="top center", textfont=dict(color=TEXT, size=13, family=FONT_HEAD),
    ...
    if selected_countries:
        for i, country in enumerate(selected_countries):
            ...
            if gdp is None or pd.isna(co2):
                continue
            pop = row["population"].values[0] if "population" in row.columns else 0
            fig.add_trace(go.Scatter(
                x=[gdp], y=[co2], mode="markers+text", name=country, text=[country],
                textposition="top center", textfont=dict(color=TEXT, size=13, family=FONT_HEAD),
                marker=dict(size=bubble_size(pop), color=EXTRA_COLORS[i % len(EXTRA_COLORS)],
...
# ═══════════════════════════════════════════════════════════════
# RUN  (gunicorn uses `server`, not this block)
# ═══════════════════════════════════════════════════════════════
